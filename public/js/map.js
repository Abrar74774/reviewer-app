...
mapboxgl.accessToken = 'pk.eyJ1IjoiYWJyYXI3NDc3NCIsImEiOiJja2F0ajhiNmswbG9jMnJsMnhjczVydTZlIn0.NW7k7GG4cIi4CUm4-wga6Q';
...
async function getPlaces() {
    ...
    const places = data.data.map(place => {
        const review = `
            ...
            ${place.reviews.map(object => `
                <div class="p-2 h-24 w-full my-2">
                    <span class="p-1 rounded-md bg-gray-300 shadow-sm" id="username">${object.username}</span>
...
function loadMap(places) {
    map.on('load', function () {
        ...
        map.on('click', 'places', function (e) {
            ...
            if (screenWidth < 1024 && mouseOnIcon) {
            ...
            console.log(selectedPlaceName, selectedPlaceCoordinates);
        });
        ...
        map.on('click', function (e) {
            if (addPlace.style.display === 'block') {
                ...
                marker.setLngLat([e.lngLat.lng, e.lngLat.lat])
                    .addTo(map);
            } else if (screenWidth < 1024 && !mouseOnIcon) {
